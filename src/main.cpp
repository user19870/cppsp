...
inline std::string noblank(const std::string& token)
{
    ...
    { // 判斷整數或浮點
        if (num == (int)num)
            return "{ auto _t = (int)" + v + "; printf(\"%d\", _t); }\n";
        else
            return "{ auto _t = " + v + "; printf(\"%g\", _t); }\n";
    }
...
size_t findMatchingEnd(const std::string& line,
                       size_t             start,
                       const std::string& h,
                       const std::string& e)
{
...
std::string runCommand(const std::string& line, bool iffunc)
{
    ...
    if (!inMultiLine && (line.find("//") == 0 || line.empty()))
        return "";
    ...
    if (inMultiLine)
    {
        ...
        // 嘗試在累積的字串中萃取參數
        std::string args =
                extractArgs(currentLine, pendingHead, pendingEnd, keywordEnd);
        if (keywordEnd)
        {
            // 找到了結尾！執行指令
    ...
    {
        // 優化：先檢查是否包含指令，避免無效搜尋
        size_t cmdPos = line.find(p.first);
        ...
        {
            const std::string& hd = note.first;
            const std::string& ed = note.second;
            // 檢查這一行是否有對應的起始符號 (例如 "(")
            ...
            bool keywordEnd  = false;
            std::string args = extractArgs(line, hd, ed, keywordEnd);
            if (!keywordEnd)
            {
                // [變更] 發現沒閉合，進入多行模式
                if (shouldRun)
...
int main(int argc, char* argv[])
{
    ...
    registerCommand("input",
                    ...
                    [](const std::string& args)
                    {
                        ...
                        while (std::getline(ss, tok, ','))
                            v.push_back(tok);
                        if (Ifiostream == false)
                            out = "printf(\"need import iostream\")";
                        if (!v.empty())
                        {
    ...
    if (argc < 2)
    {
        std::cerr << "Usage: cppsp_compiler <file>\n";
        return 1;
    }
    ...
    std::ifstream infile(cpsPath);
    std::ifstream funcfile(cpsPath);
    if (!infile)
    {
    ...
    fs::path cppPath =
            cpsPath.parent_path() / (cpsPath.stem().string() + ".cpp");
    ...
    std::ifstream fileinclude(cpsPath);
    while (std::getline(fileinclude, importline))
    {
        ...
        if (!comment && importline.find("import ") != std::string::npos)
        {
            ...
            std::string imports =
                    importline.substr(pos + 7); // "import " 長度 7
            ...
            while (std::getline(ss, header, ','))
            {
                ...
                outfile << "#include \""
                        << importFile.lexically_relative(cpsPath.parent_path())
                                   .string()
                        << "\"\n";
    ...
    while (std::getline(funcfile, funcline))
    {
        ...
        /*  if(funcline != std::string::npos){   size_t pos =
           funcline.find("@function<<"); std::string funcname =
           funcline.substr(pos + 11); // "@function(" 長度 10 funcname =
           funcname.substr(0, funcname.find(">>"));*/
    ...
    while (std::getline(infile, line))
    {
        ...
        if (line.find("#useclang") != std::string::npos)
        {
            enableclang = true;
        }
        if (line.find("#usegcc") != std::string::npos)
        {
            enableclang = false;
        }
        if (line.find("#overwrite") != std::string::npos)
        {
    ...
    fs::path    exePath =
            cpsPath.parent_path() /
            (local + cpsPath.stem().string()); // .exe後綴 : + ".exe");

    // 讀 include.ini 和 lib.ini
    std::string includeFlags = parseIni("include.ini", "-I");
    std::string libFlags     = parseIni("lib.ini", "-L");

    std::string gppCommand = "g++ \"" + cppPath.string() + "\" -o \"" +
                             exePath.string() + "\" " + extraFlags + " " +
                             includeFlags + " " + libFlags;
    if (enableclang)
        gppCommand = "clang++ \"" + cppPath.string() + "\" -o \"" +
                     exePath.string() + "\" " + extraFlags + " " +
                     includeFlags + " " + libFlags;
    if (enableoverwrite)
        gppCommand = extraFlags + " " + includeFlags + " " + libFlags;
    ...
    if (!enableoverwrite)
        int runexe = system(exePath.string().c_str());
    return 0;
}
